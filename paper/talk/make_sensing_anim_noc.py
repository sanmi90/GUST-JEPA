#!/usr/bin/env python3
"""Sparse wall-pressure sensing animation for the talk, UNCONDITIONED tf-no-c JEPA.

Copy of make_sensing_anim.py repointed at the fully unconditioned model
(outputs/session27/JEPA_d64_noc_tf encoder family + outputs/session27/decoder_noc_tf
LapFiLM decoder). Instantaneous state estimation, NO temporal predictor: a per-frame
ridge map from K=8 TCSI wall-pressure taps to the UNCONDITIONED d=64 latent is fit on
the training encounters, then applied frame-by-frame to the same representative
held-out (test_b) encounter used by the field/forecast animations. Each recovered
latent is decoded to a mid-plane vorticity field and animated next to the DNS truth,
with the 8 sensor taps marked on the airfoil.
...
Differences vs the production script (which is NOT modified):
  - DEC_CKPT -> decoder_noc_tf (local override; module D is NOT mutated on disk).
  - LAT -> outputs/session27/latents_own_jepa_noc_tf/train.npz (the unconditioned
    encoder's OWN z_full; keys case_ids / encounter_indices / impact_frame). The
    production S12_E_d64 (conditioned) latents are NOT used.
  - PICKS -> outputs/session27/pressure_noc_tf/sensor_picks_v2.json (TCSI taps
    DERIVED on the unconditioned latent; the production pressure_v2 fits are NOT
    reused, and the pressure->latent ridge is refit here on the uncond z).
  - K = 8 TCSI taps (was 16).
  - CASE -> the same representative MODERATE-G held-out encounter as the field and
    forecast animations: G+1.50_D1.50_Y+0.10 enc0 (test_b).
...
LAT = REPO / "outputs/session27/latents_own_jepa_noc_tf/train.npz"           # uncond z_full
PICKS = REPO / "outputs/session27/pressure_noc_tf/sensor_picks_v2.json"      # uncond TCSI
OUT_DIR = REPO / "outputs/session27/anim_noc_tf"
OUT_MP4 = OUT_DIR / "sensing_anim.mp4"
OUT_POS = OUT_DIR / "sensing_poster.png"
CACHE = REPO.parent / "PREVENT/data/processed/vortex-jepa/v2"

# Same representative MODERATE-G held-out encounter as the field/forecast anims.
CASE, K = "G+1.50_D1.50_Y+0.10", 0
NTAPS = 8                                    # K=8 TCSI taps
...
def main():
    ...
    sensors = json.loads(PICKS.read_text())["TCSI"][str(NTAPS)]
    print(f"K={NTAPS} uncond TCSI taps:", sensors)

    # ---- fit per-frame pressure -> UNCONDITIONED latent ridge on train ----
    lat = np.load(LAT, allow_pickle=True)
    zf = np.asarray(lat["z_full"], np.float32)
    cids = np.array([str(c) for c in lat["case_ids"]])
    encs = np.asarray(lat["encounter_indices"], int)
    Xs, Ys = [], []
    ...
    print(f"{CASE} enc{K} impact={imp}  SSIM(recovered) impact={ssim[np.where(fr==imp)[0][0]]:.2f} "
          f"H8={ssim[int(np.where(fr==np.clip(imp+8,0,119))[0][0])]:.2f} H16={ssim[j16]:.2f}")
    ...
    axes[1].set_title(f"recovered from {NTAPS} wall-pressure taps", fontsize=13, color="#3C6FB0", pad=4)
    ...
    def update(f):
        ...
        ttl.set_text(f"State estimate (unconditioned) from {NTAPS} wall-pressure taps:   "
                     f"t/c = {tc[f]:+.2f} {tag}")
        return [im0, im1]
    ...
    print("wrote", OUT_MP4, "and", OUT_POS, f"({len(fr)} frames)")
