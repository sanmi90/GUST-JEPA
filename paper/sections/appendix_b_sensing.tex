...
boundary is the same three-dimensional observability limit the field reaches
(\S\ref{sec:flow_physics}): the mid-plane state no longer determines the
dynamics there, so a \StaticInv{} has nothing to invert, and only the
accumulated pressure history carries the load through.
...
(Gupta, Chen and Wan, J. Fluid Mech. 1036, A24, 2026). When only the
...
observations, the only member with zero divergences and graceful
...
before scale: the state-dependent band fixed the impact-side divergences
