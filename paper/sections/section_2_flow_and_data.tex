...
of the gust. At matched core diameter and offset the two signs of the $|G| = 4$
gust produce near mirror-image lift transients, one a positive-first excursion
peaking near $+8.3$ and the other troughing near $-7.6$, as the sign of the gust
ratio sets the sign of the first excursion (\S\ref{sec:flow_physics}), so the two
halves of the symmetric boundary set are physically consistent. Sampling the
boundary at both signs is a genuine improvement over a one-sided $|G| = 4$ set:
it lets the sign asymmetry be measured rather than assumed
(\S\ref{sec:res_envelope}).\re
