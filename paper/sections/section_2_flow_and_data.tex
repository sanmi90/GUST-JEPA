...
\begin{figure}
  ...
  physical units with the colourbar clipped at $\pm 3$ (the intense cores saturate;
...
comprises $\NCases$ cases partitioned into training, validation (the held-out
encounters of the training cases, denoted test\_a), an
in-distribution test set (test\_b), and an out-of-distribution test set
(test\_c). The in-distribution test set is stratified to span the gust
strengths, the three core diameters, and the two source groups
(figure~\ref{fig:paramspace}); test\_c is the
$|G| = 4$ extrapolation set, sampled symmetrically in both signs of the gust
(\S\ref{sec:flow_observables}). These $\NCases$ cases are separated in
preprocessing into $\NEncounters$ encounter windows of
$120$ frames each, one per gust release: $\NTrainEnc$ training, $\NValEnc$ validation
(test\_a, the held-out last encounters of the training cases), $\NTestBEnc$ test\_b, and
$\NTestCEnc$ test\_c. The undisturbed reference case is retained in training. Behind these
encounter counts are far fewer distinct cases: the $\NTestBEnc$ test\_b encounters come
from only $\NTestBCases$ held-out cases ($\NTestBInterior$ interior and $\NTestBBoundary$
boundary tier) and the $\NTestCEnc$ test\_c encounters from $\NTestCCases$ cases, with
...
\begin{figure}
  ...
  (test\_b, $\NTestBEnc$ encounters from $\NTestBCases$ cases, split into interior
  and boundary tiers), and the out-of-distribution extrapolation set (test\_c,
