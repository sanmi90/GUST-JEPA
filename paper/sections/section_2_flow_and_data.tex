...
release and appear only here. The \TestSplit{} set is stratified to span the gust
...
boundary tier) and the $\NTestCEnc$ \BoundarySplit{} encounters from $\NTestCCases$ cases, with
...
\begin{figure}
  ...
  training ($\NTrainEnc$ encounters), the stratified \TestSplit{} set
  ($\NTestBEnc$ encounters from $\NTestBCases$ cases, split into interior
  and boundary tiers), and the \BoundarySplit{} extrapolation set
