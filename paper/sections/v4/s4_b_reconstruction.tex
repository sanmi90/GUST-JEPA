...
test\_b through identical-capacity frozen decoders with the Wang SSIM
...
head (\TcSsimNbCLN{} for CLN against \TcSsimNbCLW{} for CLW), the
...
\PoneLowdJepa{} (flagship lineage) and \PoneLowdAero{} (coupled-objective
arm) against \PoneLowdFukami{} for the reconstructive baseline, three
seeds each with non-overlapping ranges; the CLN lineage trained through
the direct predictor is close to dimension-insensitive on the peaks, from
\LadRexpredDFour{} at $d = 4$ to \PoneRexpredPeak{} at $d = 32$ (three
seeds at both ends), where the flagship lineage declines with $d$ (from
...
$d = 32$ on test\_b): the lift information in the state is
...
\begin{figure}
  ...
  \caption{Decode floor on the representative test\_b encounter
...
\begin{figure}
  ...
  \caption{The dimension axis on test\_b. (a)~Peak-region lift $R^2$
