...
\rb{K}{S4B2-01}{The two-tier compression statement, which is the paper's answer on how small the state can be. \S6 quotes it directly.}The dimension axis splits into two tiers, and the split is the paper's
...
state needs $d \geq 16$ while the lift saturates by $d \approx 4$ to $8$. This
decode-fidelity bar is distinct from, and stricter than, the wake-readability
threshold of \S\ref{sec:res_code}, where a wake $R^2$ of one half is reached
from eight coefficients.\re

\rb{K}{S4B2-02}{The probe-dilution control that disciplines the $d = 4$ claim. Without it the compression result would be over-read.}The probe-dilution control disciplines how the first tier may be read
...
distributed and partly nonlinear above.\re

\begin{figure}
  \centering
  \includegraphics[width=\linewidth]{sections/figures/results/fig_dimension_race_v4.pdf}
  \caption{\rb{K}{S4B2-C1}{Three panels including the control, with the seed provenance stated. Self-contained.}The dimension axis on the \TestSplit{} set. (a)~Peak-region lift $R^2$
  ...
  lift-focused lineage rises with $d$: the wake tier is not compressible.\re}
  \label{fig:dimrace}
\end{figure}
