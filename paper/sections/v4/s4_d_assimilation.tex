...
(figure~\ref{fig:centerpiece}a,b). Assimilation is decisive at impact:
...
(figure~\ref{fig:centerpiece}c), and its top rung is unexpected:
...
\StaticInv{} posts the ladder's single divergent encounter (impact-phase $R^2$ below $-1$,
the criterion we use for divergent throughout); on the
extrapolation boundary, where the \TwoStageKF{} reaches
\PoneTwoStageCovImpactC{} median $R^2$ on the $|G| = 4$ \BoundarySplit{}
encounters with no divergent case and a positive relaxation
...
What the training objective confers survives to the assimilation stage, and
...
state-dependent noise removes the divergent encounters entirely; in
...
\begin{figure}
  ...
  \caption{Per-family end-to-end comparison (\TestSplit{} set, $42$ encounters, single
...
\begin{figure}
  ...
  \caption{The assimilation-versus-dimension grid (\TestSplit{} set, per-family end-to-end stacks,
