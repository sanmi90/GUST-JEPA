...
(figure~\ref{fig:tracking}). Assimilation is decisive at impact:
...
\begin{figure}
  ...
  \caption{Phase-resolved lift tracking on the \TestSplit{} set ($n = 42$
  encounters, $K = 8$ taps, delay $10$, $N = 64$ members, every-frame pressure).
  Lift error per phase in physical units (a) and in $R^2$ (b) at the
  val-calibrated band: the relaxation $R^2$ collapse is a variance artifact, the
  physical error the honest phase metric. The estimator ladder, its noise
  calibration and the fixed-lag smoother are in
  appendix~\ref{app:calibration} (figure~\ref{fig:calibration}).}
  \label{fig:tracking}
\end{figure}
