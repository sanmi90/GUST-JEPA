...
dynamically relevant wake observables, the enstrophy and signed circulation in
particular, linearly readable in the latent, and the drift
...
latent so that the undisturbed shedding orbit remains a single loop, the
...
the same way, shown as consistent tracking rather than advanced as a validated forecast.
...
Third, we attribute the wake gain to its cause. The
...
supervision, so we phrase the result as a property of the
...
objective trained with wake-observable supervision rather than to the architecture. The
same predictive state is also the most recoverable of the three
from sparse wall pressure, so it is both the most wake-readable and the most
wall-recoverable of the three (\S\ref{sec:res_observability}), and it is the latent
dynamics function a controller would plan against, a closed-loop demonstration of
which lies beyond the present rollout accuracy (\S\ref{sec:discussion}).
