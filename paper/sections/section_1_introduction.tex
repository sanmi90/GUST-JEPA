...
held-out cases the wake-supervised predictive latent renders the wake linearly
readable where the others do not: its representational wake-enstrophy $R^2$ is
$\NumReprWakeJepaTf$ where the reconstructive and linear baselines are at or below
zero, and the per-encounter advantage survives the pre-registered encounter-level
multiplicity correction. A control shows the readability is supplied by the
wake-observable supervision, with the predictive objective distinguished by
preserving it where the reconstructive objective suppresses it and by rendering the
wake forecastable. We phrase the result as linear decodability rather than exclusive
...
from sparse wall pressure, so it is both the most wake-readable and the most
wall-recoverable of the three (\S\ref{sec:res_observability}), and we relate the predictor to the latent
