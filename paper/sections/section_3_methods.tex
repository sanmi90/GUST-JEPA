...
\begin{table}[t]
  ...
  the gust parameters $\cvec = (G,D,Y)$ and the within-encounter phase enter only
  ...
  auxiliary heads read observables off the encoder output (a lift head,
  ...
  \begin{tabular}{l l l}
    ...
    conditioning    & none & AdaLN-Zero on $(\cvec, \phi)$, rotary time \\
...
observable for evaluation. The predictor
...
% ALTERNATIVE to fig:jepa_architecture, block-diagram style (kept so the
% authors can choose between the two). Remove one before submission.
\begin{figure}[t]
  \centering
  \includegraphics[width=0.95\linewidth]{sections/figures/tikz/fig1b_jepa_blockdiagram.pdf}
  \caption{Block-diagram view of the JEPA, partitioned into the physical (DNS)
  and computational domains in the style of a data-driven control schematic. The
  encoder builds the reduced state $\zvec_t = E_\theta(\omegaz^t)$ and the
  predictor advances it forward in time (the forward model), with the gust
  parameters $\cvec = (G,D,Y)$ entering only the predictor. Two observable heads
  read the lift and an $80$-dimensional wake spectrum off the latent (the wake
  penalty), supervised by the DNS observables; the visualisation decoder is
  trained later and never enters the latent-prediction loss.}
  \label{fig:jepa_blockdiagram}
\end{figure}
