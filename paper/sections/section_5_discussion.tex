...
in-distribution median matters and latency is irrelevant, the static
delay-embedded inverse is the strongest and simplest tool, and no dynamics
are needed. The moment the tails, the extrapolation boundary or continuous
state tracking matter, the filters take over: online, the two-stage
schedule is the working choice, the forecast-derived noise carrying the
...
degradation under subsampled updates; every-frame pressure is load-bearing
...
matched-predictor merit between the full predictive state and the objective-free
supervised control does not replicate across retrains. A seed study of the fitted
merit, run under the residual U-Net operator as a stability check, cautions
against over-reading fitted-merit differences: over three seeds the supervised
...
($\pm \SeedMeritSdJepaVec$), and the family means overlap. The narrow lead the
predictive state holds under its suited transformer operator (\S\ref{sec:res_closure})
is therefore not decisive on its own. The defensible dynamical claim rests instead
on the as-built model and its rollout: the flagship's own predictor forecasts
markedly better than a matched operator extracts from any latent, and the
multi-step objective buys long-horizon stability and lower drift, not a larger
