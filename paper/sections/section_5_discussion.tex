...
The comparison has a direction, read off the chain of tasks a wall-borne reduced
state must perform. On the decoded field the three
...
basis loses the load at the impact instant and neither it nor the published-recipe
reconstruction
reads the wake; and it widens at every step toward deployment. Through the impact
the published-recipe reconstruction's forecast collapses and the linear basis
fades; recovering
...
boundary every filter is stressed, but the predictive filter posts by far the
fewest innovation-consistency failures ($\VdivGFour$ of encounters against
$\FfDivAeWakeGFour$ to $\FfDivFukamiGFour$ for the others). The predictive state holds furthest down the chain,
and that is the design rule stated operationally: a state earns
...
wake enstrophy at the readout horizon, so a state can look accurate on the load while
...
the POD coefficients do not achieve comparable linear readability of the
wake-enstrophy target at any dimension we tested.
...
estimator raises every family without changing their order (table~\ref{tab:recovery}; supplementary
...
inverse, whose in-distribution impact median it matches rather than beats
(\S\ref{sec:res_da}), realising the online-estimation pathway the extreme-aerodynamic control
...
the ensemble collapses. We conjecture, beyond what the exhibits establish, that
the forward model carries gain along data-unconstrained directions it
never learned to damp, and that those inject spread-consuming noise during
assimilation; what the record does establish is that the innovations are
temporally correlated rather than white
(\S\ref{sec:res_envelope}). Either reading points to a calibrated process and
...
three collapsing), the predictive latent is tighter
...
the encoder observes a single mid-plane vorticity slice that captures a
substantially smaller fraction of the three-dimensional content there
(\S\ref{sec:flow_physics}), so the \BoundarySplit{} set marks where that
representation becomes incomplete, not a parametric-generalisation claim, and the
filter meets the same limit from the sensing side (\S\ref{sec:res_envelope}). Fourth, the
...
A further direction concerns the wake target itself. The present descriptor fixes
where the wake vortices sit and at which scales their energy resides, but its
coordinates are generic. Weak-form dominant-balance analysis \citep{ahnert2026}
suggests a physically grounded alternative: inner products of the conservation-form
vorticity-transport terms against compactly supported test functions, so that each
supervised coordinate is the local magnitude of a governing mechanism, evaluated
without differentiating the data. Such a target is capacity-matched to the present
descriptor by construction and would align the protected subspace of
\S\ref{sec:res_rollout} with the mechanisms themselves; the controlled substitution
