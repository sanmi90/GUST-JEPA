...
supervised control does not replicate across retrains. Read through a common
matched operator, the merit does not cleanly separate the families: over three
seeds the supervised control is seed-fragile (spread $\SeedMeritSdSupOnly$, with
one healthy retrain in three collapsing), the predictive latent is tighter but
not the sharpest ($\pm \SeedMeritSdJepaVec$), and its seed-mean sits level with
the matched reconstructive control ($\SeedMeritJepaVec$ against $\SeedMeritAeWake$),
because that common operator is off-class for the transformer-trained state and
understates it. The defensible dynamical claim rests instead on the as-built
model and its rollout: the flagship's own predictor forecasts markedly better
than any common operator extracts from any latent, and the multi-step objective
buys long-horizon stability and lower drift, not a larger instantaneous
readability margin. Second, the wall-normal offset $Y$ remains the least
