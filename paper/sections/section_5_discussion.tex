...
encounter and extends the usable gust-intensity range beyond single-frame
recovery, realising the online-estimation pathway the extreme-aerodynamic control
literature has named \citep{fukami2024control,mousavi2025,eldredge2025guide}. We
are deliberate about its limits. Its recoverability advantage over a raw
...
observable supervision, and the single-seed tie in matched-predictor merit between
the full predictive state and the objective-free supervised control does not
replicate across retrains. Over three seeds the supervised control's merit is
seed-fragile (one healthy retrain in three collapses) while the predictive
latent's is stable, so the defensible dynamical claim is that the predictive
objective supplies seed-robust forecastability and long-horizon stability, not a
larger instantaneous margin. Second, the wall-normal offset $Y$ remains the least
