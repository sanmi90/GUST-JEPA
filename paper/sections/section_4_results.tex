...
The model is fully unconditioned: no gust parameters enter the encoder or the
predictor. We distinguish two questions. Does the latent, encoded directly from a
held-out field, carry each observable (representation quality, the headline,
table~\ref{tab:closure}(a))? And does the unconditioned predictor, rolled out from
the impact frame, track it (forecast quality, reported as secondary,
...
sixteen frames after impact, the unconditioned predictive latent encodes the wake
enstrophy at $R^2 = 0.71$ (figure~\ref{fig:fig4_closure}), essentially matching the
$0.75$ of a model that does receive the gust parameters (\S\ref{sec:res_controls}),
while the reconstructive autoencoder and the linear basis are at $0.06$ and below.
Removing the gust parameters entirely therefore costs almost nothing in what the
latent encodes: the predictive objective alone builds a state that keeps the wake,
the load-bearing structure that reconstruction smooths away. The forces and the
circulations are read off the same latent cleanly, with $C_L$ at $R^2 = 0.88$, the
two circulations near $0.78$ to $0.85$, and the drag at $0.69$; the mean over the
six observables is $0.51$. The one weak observable is the wall-normal impulse $I_y$
($R^2 = -0.86$), a mid-plane vorticity diagnostic that even the energy-optimal
linear basis captures only modestly (\S\ref{sec:flow_observables}), so its weakness
is a property of the observable rather than of the predictive latent. In native
units the predictive-latent representational wake-enstrophy error is $35.1$, against
the larger errors the reconstructive and linear encoders incur on the same field
(table~\ref{tab:closure}(a)).

\begin{figure}
  \centering
  \includegraphics[width=\linewidth]{sections/figures/results/fig4_closure.pdf}
  \caption{Representational closure of the unconditioned predictive latent at
  $H = 16$ after gust impact, the headline result. (a) Held-out $R^2$ of a fixed
  linear probe applied to the simulation-encoded latent (no rollout) for each of
  the six observables, on the in-distribution test set (test\_b, $n = 42$); the
  dashed line marks the mean over the six and the one weak observable, the
  mid-plane impulse $I_y$, is shaded light. (b) The wake-enstrophy $R^2$ across
  families: the unconditioned predictive latent ($0.71$) against the reconstructive
  autoencoder ($0.06$) and the linear basis ($-0.17$), each baseline at its
  least-bad latent dimension; the dotted line marks the conditioned model's $0.75$,
  which the unconditioned latent essentially matches. The wake is the discriminator,
  and the predictive latent keeps it with no privileged knowledge of the gust.}
  \label{fig:fig4_closure}
\end{figure}

The rollout points the same way and is read as confirmatory, not as the headline
(table~\ref{tab:closure}(b) and figure~\ref{fig:traces}). Rolling the unconditioned
predictor recursively from the impact frame and probing at $H = 16$, the
wake-enstrophy forecast $R^2$ is $0.56$, well above the predict-the-mean floor,
against $-0.48$ for the reconstructive autoencoder and $-0.09$ for the linear basis,
both worse than forecasting the held-out mean. The reconstructive autoencoder's wake
forecast is non-monotonic in capacity and worst at the matched $d = 64$: a
reconstruction objective constrains only the latent directions its decoder reads,
leaving proportionally more of the latent unconstrained as $d$ grows, so the
autoregressive rollout has more room to drift off the wake manifold
(\S\ref{sec:res_drift}). The instability is seed-borne rather than a
single-checkpoint artefact, with an eightfold larger seed spread than the predictive
objective at matched architecture in the conditioned-model control
($0.16 \pm 0.27$ against $0.46 \pm 0.03$; table~\ref{tab:controls_2x2}). The
unconditioned rollout also leads the mean over the six observables ($0.36$ against
$0.15$ for both the $d = 64$ reconstructive and linear bases) and the negative
circulation. On the OOD test\_c set ($G = +4$) the predictive wake-enstrophy
forecast remains positive ($R^2 = 0.17$ at $H = 16$, and higher at $H = 8$) while
the integrated forces collapse, consistent with the three-dimensional observability
boundary of \S\ref{sec:flow_physics}.

\begin{figure}
  \centering
  \includegraphics[width=\linewidth]{sections/figures/results/figA_traces.pdf}
  \caption{The unconditioned rollout tracks the encounter, shown qualitatively.
  ...
  simulation is the bold reference; the rollout of the unconditioned predictor,
  started at the impact frame with no gust input, is the coloured line, and the
  dashed line is the reconstruction from the encoded latent. The rollout tracks the
...
shows the predictive latent carrying the smaller wake-enstrophy error on $31$ of
$42$ held-out encounters under direct encoding (paired mean improvement $43.1$,
$95\%$ CI $[23.5, 66.0]$, one-sided sign $p = 1.4 \times 10^{-3}$). The marginal
per-observable bootstrap intervals ($2000$ resamples, reported in full in
Appendix~\ref{app:reg}) are by contrast wide, because the same shared difficulty
inflates them; the paired test, not the marginal interval, is therefore the
headline. This paired wake advantage is robust to the encoder seed: across three
...
reconstructive (mean $\pm$ s.d.\ over seeds). All six observables are reported in
both modes, not a selected subset. The ordering is not uniform: the predictive
latent leads on the wake observables (enstrophy and negative circulation), and this
representational wake advantage is the one that survives both case-level clustering
and a family-wide correction over the twelve paired tests; the forecast advantage
points the same way but survives neither (table~\ref{tab:paired_closure}). The
reconstructive \emph{representational} wake-enstrophy $R^2$ interval is entirely
negative ($[-4.62, -0.12]$), a clean statement that the reconstructive latent fails
to carry wake structure on held-out cases. This separation is confirmed by the
...
where the families separate consistently. The paired test contrasts the predictive
and reconstructive encoders directly; the unconditioned model the paper reports
encodes the wake at $R^2 = 0.71$ on this same held-out set, essentially the
conditioned value (\S\ref{sec:res_controls}), so the wake-carrying property does not
depend on the gust parameters.

\begin{table}
  \centering
  \caption{Parameter-only floor at $H = 16$ after impact. A cross-validated
  kernel-ridge regressor maps the gust parameters $\cvec = (G,D,Y)$ directly to
  each observable, with no latent input; $R^2$ is reported on each split. This is
  a model-free floor, and it carries particular force for the unconditioned model,
  which never receives $\cvec$ at all: the last column gives the unconditioned
  predictive latent's representational closure (probe on the simulation-encoded
  latent, test\_b, table~\ref{tab:closure}(a)). On wake enstrophy, the observable
  that carries the result, the parameter floor is low (test\_b $R^2 = 0.17$),
  whereas the unconditioned latent encodes the wake at $0.71$; the latent therefore
  carries wake state that the generative parameters alone do not supply. At the
  impact frame, by contrast, the same parameter floor on wake enstrophy reaches
  $R^2 \approx 0.70$, because the just-released gust nearly determines the
  instantaneous state and the gap opens only as the wake evolves. On the lift and
  the negative circulation, which track the gust forcing more directly, the floor
  is high; out of distribution (test\_c, $|G| = 4$) it turns negative for the wake,
  drag, and positive-circulation observables.
  }
  \label{tab:conditioning_floor}
  \small
  \fittab{\begin{tabular}{l c c c c c}
    \toprule
    Observable & $n_{\text{train}}$ & train $R^2$ & test\_b $R^2$ & test\_c $R^2$ & uncond.\ repr.\ test\_b $R^2$ \\
    \midrule
    $C_L$          & 226 & 0.922 & 0.707  & 0.741  & 0.88 \\
    $C_D$          & 226 & 0.886 & 0.521  & -3.993 & 0.69 \\
    $I_y$          & 226 & 0.651 & 0.417  & 0.207  & -0.86 \\
    wake-enstrophy & 226 & 0.571 & 0.173  & -0.103 & 0.71 \\
    circ.\ pos.\   & 226 & 0.845 & 0.166  & -1.884 & 0.78 \\
    circ.\ neg.\   & 226 & 0.832 & 0.795  & 0.214  & 0.85 \\
    \bottomrule
  \end{tabular}}
\end{table}

The parameter-only floor (table~\ref{tab:conditioning_floor}) confirms that the
wake closure is not something the gust parameters supply on their own, which is
the sharpest statement available for a model that never sees those parameters.
Mapping $\cvec = (G,D,Y)$ to each observable at $H = 16$ with a cross-validated
kernel-ridge regressor, the floor on wake enstrophy is low (test\_b $R^2 = 0.17$,
rising to $0.31$ only when a within-encounter shedding-phase proxy is added), well
below the unconditioned latent's representational wake closure ($0.71$). The
contrast with the impact frame is the mechanism: there the same parameter floor on
wake enstrophy reaches $R^2 \approx 0.70$, because the just-released gust nearly
...
dynamics, which is the information the predictive latent carries and the parameters
do not. The floor is by construction strong on the observables that track the gust
forcing directly, the lift and the negative circulation: the predictive advantage
is specific to the spatially distributed wake, not uniform across observables. The
wake claim therefore rests on the representational closure
...
with the parameter floor establishing that the gust parameters do not supply the
wake state the latent encodes. A conditioned variant that adds $\cvec$ to the
predictor is reported as a control in \S\ref{sec:res_controls}
(table~\ref{tab:jepa_dimsweep}): the conditioning sharpens the in-distribution
forecast but leaves the representation essentially unchanged, which is why the
unconditioned model carries the result.
...
predictive latent \emph{family} as configured keeps the wake better than the
...
rollout stays on the manifold (\S\ref{sec:res_drift}). The wake supervision is the
more pointed confound, since the largest part of the reported advantage is on the
wake observables that the predictive encoder alone was supervised on. Isolating the
objective requires the controls in table~\ref{tab:controls_2x2}, which were run on
the conditioned model as the most direct objective-versus-architecture crossing: a
...
The decisive question is whether the predictive objective keeps the wake better than
the reconstructive one once the architecture and the wake supervision are held in
common; the unconditioned model the paper reports inherits this attribution, since
the conditioning leaves the representation essentially unchanged (third control
below).

\begin{table}
  \centering
  \caption{Objective $\times$ architecture controls (decisive experiment), run on
  the conditioned model as a control. Held-out wake-enstrophy closure and mean
  closure ($R^2$ at $H = 16$, test\_b, Markov rollout, mean $\pm$ standard
  deviation over three encoder seeds) for the four cells crossing \{predictive,
  reconstructive\} with \{CNN, CNN$+$ViT\} at $d = 64$, with the auxiliary heads
  (the instantaneous lift plus the $80$-dimensional wake head) matched across all
  four cells, plus a predictive variant with the wake head removed. Predictive
  beats reconstructive on wake closure at both architectures (predictive versus
  reconstructive at CNN$+$ViT, and again at CNN), so the objective is not a confound
  of the architecture; removing the wake head from the predictive model collapses
  wake closure below the floor, so the wake supervision is also necessary. The CNN
  and CNN$+$ViT columns do not separate, so the ViT is not the driver. These cells
  isolate the objective and the supervision; the unconditioned configuration the
  paper reports inherits the same attribution.}
...
channel without collapsing any. The third control is the one the paper turns on: it
removes the gust-parameter conditioning from the predictor (last row), so the model
sees no $\cvec$ anywhere. This is the unconditioned configuration reported
throughout. It leaves the representation essentially intact, because the encoder is
unconditional in either case: the representational wake MAE is $34.21$ against
$29.83$, and the representational wake $R^2$ is $0.71$ against $0.75$. What the
conditioning buys is in-distribution forecast sharpness, not representation: the
in-distribution forecast wake $R^2$ is lower without it, and the conditioned model
extrapolates the forecast to the unseen $|G| = 4$ gust strength more robustly. The
trade is therefore favourable for the central claim: removing the parameters costs
almost nothing in what the latent encodes, so the unconditioned predictive objective
alone produces a state that keeps the wake, which is the property a downstream
estimator reads.
