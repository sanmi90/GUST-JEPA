...
Document compiled 2026-05-27, updated 2026-05-28 for split_v2 (see "Split
v2 update" below).

## Split v2 update (2026-05-28, HANDOFF D130)

**This rerun targets `split_v2.json`, not v1.** The v2 partition introduces
a 4-way train/val/test_b/test_c protocol with the following changes from v1:

- **Test B expanded from 6 to 10 cases** under stratification criteria C1-C5
  (archived inside `configs/splits/split_v2.json` under `test_b_criteria`).
  10 cases x ~4 encs = 42 test_b encounters.
- **`test_a` renamed to `val`** for semantic clarity. Mechanism unchanged
  (periodic: 4+2 encs; run3: 3+1 encs).
- **Test C unchanged** (4 periodic cases at G=+4).
- **Two-tier Test B reporting**: each test_b case carries
  `n_train_neighbors_d2` and `tier` in {interior, boundary}. Current split:
  5 interior + 5 boundary.
- **Uncertainty protocol** (mandatory per D130): bootstrap CI (n=2000) on
  test encounters + 3-seed encoder variance + 5-fold probe CV on the
  readout step. Train + val + test_b + test_c metrics reported on every
  headline for the no-overfitting check.

The split file is generated by `build_split_manifest_v2.py` (parallel to
the v1 generator). v1 stays untouched on disk for older-session
reproducibility but no new analysis uses it.

Final v2 summary (verified 2026-05-28):

| Bucket | Cases | Encounters |
|---|---|---|
| Train | 70 | 226 |
| Val (within-train-case) | -- | 86 |
| Test B (interior) | 5 | 26 |
| Test B (boundary) | 5 | 16 |
| Test C (OOD G=+4) | 4 | 24 |
| **Total** | 84 | 378 |

Every reference to `split_v1.json` below should be read as `split_v2.json`
for the v2 rerun. Every script that takes `--split` should be invoked with
`configs/splits/split_v2.json`. The `partition_version` string in the
...
- `CLAUDE.md`, `HANDOFF.md` (D130 documents the v2 design)
- `SESSION18 PLAN.md`, `SESSION18_B1_PROTOCOL.md`
- `configs/preprocessing.yaml`
- `build_split_manifest.py` (v1 generator, kept for reproducibility)
- `build_split_manifest_v2.py` (v2 generator; canonical for the rerun)
- `configs/splits/test_b_v2_proposal.json` (Test B selection analysis archive)
- All of `src/`
...
## Stage 2: Split manifest (v2)

| Artifact | Producer | Inputs |
|---|---|---|
| `configs/splits/split_v2.json` | `build_split_manifest_v2.py` | inventory |

Split policy v2 (hardcoded in `build_split_manifest_v2.py`):
- G == +4 -> test_c; case_id in `TEST_B_V2_CASE_IDS` (10 stratified interior cases) -> test_b; else train
- Train cases: periodic uses encounters [0,1,2,3] for train, [4,5] for val; run3 uses [0,1,2] for train, [3] for val
- Baseline.h5 stays in train + val with `is_calibration_reference: true`
- Each test_b case gets `n_train_neighbors_d2` and `tier` in {interior, boundary}

```bash
python build_split_manifest_v2.py
```
Compute: seconds. Verify `summary.n_cases_total == 84`, the 10 test_b case list
matches `TEST_B_V2_CASE_IDS`, and the interior/boundary count is 5/5.

The v1 generator (`build_split_manifest.py` producing `split_v1.json`) is kept
on disk but NOT used by the v2 rerun. To regenerate v1 explicitly: same script,
unchanged.
...
- [ ] `configs/splits/split_v2.json` has `n_cases_total=84`, `n_cases_train=70`, `n_cases_test_b=10` (5 interior + 5 boundary), `n_cases_test_c=4`
- [ ] `outputs/data_pipeline/v1/manifest.json` (or `v2` if you bumped partition_version) has new `train_stats.std` reasonable for the new 70-train-case dataset (will shift ~3-5% from the previous 60-train-case stats)
- [ ] Production JEPA SL decoder val SSIM_mean >= 0.50 (D99-equivalent gate, on the new val set)
- [ ] B1 Fukami AE gate passes for each d (val SSIM_mean >= 0.60 OR ratio_mean < 2.0; `scripts/session18/verify_fukami_gate.py` with val instead of test_a)
- [ ] Train + val + test_b + test_c reported for EVERY headline metric (no-overfitting check per D130)
- [ ] Bootstrap CIs (n=2000) computed on test encounters for every headline
- [ ] 3-seed encoder variance reported alongside the bootstrap CIs
- [ ] 5-fold probe CV reported for probe R^2 claims on G, D, Y
- [ ] Test B aggregates reported separately for interior (5 cases) and boundary (5 cases) tiers
- [ ] Session 17 Exp 2 Markov closure replicates the qualitative direction (Markov-only matches or beats Full-context at H <= 16 on physical observables) under the new split
