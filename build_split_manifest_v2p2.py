"""Generate split_v2.2 from raw_cases_inventory_v2p2.yaml.

Changes from v2.1 (`build_split_manifest_v2p1.py`):

- **17 new run4 cases added** covering G in {-4.0, -2.0, -1.0, -0.5, -0.25}
  at D in {0.5, 1.0} and Y in {+0.10, -0.10}.
- **Test C extended to |G| = 4.0** (both signs). The 4 new G=-4.0 cases join
  the existing 4 G=+4.0 cases; Test C now has 8 cases providing a symmetric
  strong-gust extrapolation set.
- **G_GRID extended with -0.25** to support Manhattan-distance neighbor counts
  for the new cases added to train.
- **Test B unchanged** (same 10 cases from v2.1).
- **13 run4 cases go to train** (G in {-0.25, -0.5, -1.0, -2.0}).
  Encounter assignment matches run3/run4 convention: indices [0,1,2] train,
  [3] val.
"""
...
INVENTORY_PATH = REPO / "data_manifest" / "raw_cases_inventory_v2p2.yaml"
OUTPUT_PATH = REPO / "configs" / "splits" / "split_v2p2.json"

# Test B: unchanged from v2.1 — 10 cases.
TEST_B_CASE_IDS = {
    "G+0.50_D1.50_Y+0.00",
    "G-0.50_D1.00_Y-0.40",
    "G+1.00_D0.50_Y+0.40",
...
TEST_B_CRITERIA = {
    ...
    "C5_source_pooling":
        "Periodic and run3/run4 cases mixed without per-source quota.",
}

# G_GRID extended with -0.25 (new in run4) and -4.0 / +4.0 excluded (Test C).
...
def case_split(case_id: str, G: float) -> str:
    if abs(G) == 4.0:
        return "test_c"
...
def encounter_assignment(split: str, source: str, n_encounters: int):
    ...
    if source == "periodic":
        return [0, 1, 2, 3], [4, 5]
    # run3 and run4 both have 4 encounters
    return [0, 1, 2], [3]
...
def main():
    ...
    for c in inv["cases"]:
        ...
        split = case_split(case_id, G)
        train_enc, val_enc = encounter_assignment(split, source, n_encounters)
    ...
    # Manhattan-distance neighbor counts for two-tier Test B labelling.
    ...
    for cid, c in cases_out.items():
        ...
        nbrs = sum(
            ...
            and abs(gj - gi) + abs(dj - di) + abs(yj - yi) <= 2
            and (gj, dj, yj) in train_grid_coords
        )
        c["n_train_neighbors_d2"] = nbrs
        c["tier"] = "interior" if nbrs >= 4 else "boundary"

    for tb in TEST_B_CASE_IDS:
        assert tb in cases_out, f"Test B case {tb!r} not in inventory"
    ...
    n_interior = sum(1 for c in cases_out.values() if c.get("tier") == "interior")
    n_boundary = sum(1 for c in cases_out.values() if c.get("tier") == "boundary")
    n_calib = sum(1 for c in cases_out.values() if c.get("is_calibration_reference"))

    manifest = {
        "manifest_version": "split_v2.2",
        ...
        "split_policy": {
            "test_c_criterion": (
                "|G| == 4.0 (8 cases: 4 G=+4.0 periodic + 4 G=-4.0 run4); "
                "symmetric strong-gust extrapolation set; never used for selection"
            ),
            "test_b_criterion": (
                "10 cases unchanged from v2.1 under stratification criteria C1-C5 "
                "(see test_b_criteria); cases tagged tier=interior if "
                "n_train_neighbors_d2 >= 4, else tier=boundary"
            ),
            "val_criterion": (
                "within training cases: last 2 of 6 encounters (periodic) or last 1 of 4 "
                "encounters (run3/run4); model-selection signal"
            ),
            "baseline_role": (
        ...
        "test_b_criteria": TEST_B_CRITERIA,
        ...
        "summary": {
            ...
            "n_cases_calibration_reference": n_calib,
    ...
    print()
    print("Test C cases (8):")
    for cid in manifest["test_c_cases"]:
        c = cases_out[cid]
        print(f"  {cid}  src={c['source_group']}  n_enc={c['n_encounters_full']}")
    print()
    print("Run4 cases -> split assignment:")
    for cid, c in sorted(cases_out.items()):
        if c["source_group"] == "run4":
            print(f"  {cid:30s}  split={c['split']:7s}  train_enc={c['train_encounter_indices']}")
