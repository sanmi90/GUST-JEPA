...
| M2b | Parameter-only floor (s3 diagnostics prose) | session-23-era exp_conditioning_floor_plus.py output | scripts/session23/exp_conditioning_floor_plus.py | PRE-v2.2 GENERATION; no v2p2 artifact found; the claim survives only as qualitative Methods sentences (section_3_methods.tex:235-245), no macro-bound number, no figure | RE-RUN on v2p2 (Track M2 this session): kernel ridge (G, D, Y) to each observable at H = 16, train-fit, held-out-scored; verify "low on wake enstrophy, high at impact" |
