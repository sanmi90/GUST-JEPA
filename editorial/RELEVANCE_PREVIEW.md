...
## sections/section_3_methods.tex  (S3, 11 blocks)
...
- **S3-08** (para, L100-100) Two diagnostics turn the closure result into a mechanism. The first is the latent drift, which addresses a discrepancy the closure numbers alone cannot resolve:
- **S3-09** (para, L102-102) The second diagnostic is a parameter-only floor, and it decides whether the central closure result means anything at all. Each encounter is generated by three n
- **S3-10** (para, L104-104) The predictive encoder differs from the reconstructive baseline in three ways at once, its objective, its architecture and its auxiliary supervision, so a bare 
...
## sections/v4/s3_4_estimators.tex  (S34, 10 blocks)

- **S34-C1** (caption, L18-48) The predict-correct loop at deployment, and the only route by which information reaches the state once the vorticity field is gone. \emph{Predict.} An ensemble 
- **S34-01** (para, L52-80) At deployment the vorticity field is unavailable, and the wall pressure at a single instant does not determine the state: one pressure frame recovers little of 
- **S34-02** (para, L82-108) Every estimator in this paper solves the same discrete state-estimation problem on the frozen coefficient state. Write $\zvec_t \in \mathbb{R}^{d}$ for the enco
- **S34-03** (para, L110-123) Two assimilation protocols are used, and each result states which one it follows. Under the envelope protocol, assimilation begins $\AssimPre$ frames before imp
- **S34-04** (para, L128-149) The base filter is a stochastic ensemble Kalman filter in the perturbed-observation form of \citet{burgers1998}, following \citet{evensen2003}, with $N = \Filte
- **S34-05** (para, L151-181) Write $\bar{\zvec}^{f}_t = N^{-1}\sum_i \zvec^{f,(i)}_t$ for the ensemble mean of the forecast, $X^{f}_t = [\zvec^{f,(i)}_t - \bar{\zvec}^{f}_t]_i$ for the $d \
- **S34-06** (para, L183-197) Multiplicative inflation $\zvec^{f,(i)} \leftarrow \bar{\zvec}^{f} + \rho\,(\zvec^{f,(i)} - \bar{\zvec}^{f})$ is applied before the update. It rescales the ense
- **S34-07** (para, L202-237) This configuration replaces both ingredients of the base filter with their simplest stable counterparts, and it exists to separate what the estimator contribute
- **S34-08** (para, L242-270) The \FnoiseKF{} keeps the encoded observation \eqref{eq:eobs} and replaces the member propagation by the \DirectFC{} of \S\ref{sec:methods_rex}: each member is 
- **S34-09** (para, L272-305) The single scalar $c$ is the filter's only tuned quantity and it carries two independent calibrations, both on validation data alone. The production value $c = 

## sections/v4/s3_5_protocol.tex  (S35, 2 blocks)

- **S35-01** (para, L7-45) Every encounter is scored in three phases anchored to its impact frame: lead-in $[t_{\mathrm{imp}} - 8,\, t_{\mathrm{imp}})$, impact $[t_{\mathrm{imp}},\, t_{\m
- **S35-02** (para, L47-73) Uncertainty enters the reported numbers from six places. Two are sampling: a bootstrap over held-out encounters, and a clustered bootstrap over whole cases for 
...
- **S4D-04** (para, L68-77) When a quarter-convective-time latency is acceptable, the smoother changes the deployment rule. The fixed-lag recursion of the fixed-lag smoother of appendix~\r
...
- **APA-C15** (caption, L437-446) Per-estimator configuration (Gupta \emph{et al.} 2026, table~1 analogue). All fits use the training split only; $\rho$ and the two $c$ calibrations use validati
- **APA-C16** (caption, L473-479) The six sources of uncertainty, with the unit each one varies or resamples, its count, and the results that carry it. No single number carries all six: which ap
...
## sections/appendix_b_sensing.tex  (APB, 13 blocks)
...
- **APB-03** (para, L45-67) The model-conditioned placement above is built one sensor at a time. Starting from an empty set, each step adds the sensor that most improves a ridge prediction
- **APB-04** (para, L70-79) At a single frame the wall is blind to the wake-circulation direction of the predictive state (\S\ref{sec:res_wall}); over the thirty-frame window the blind coo
- **APB-05** (para, L82-89) Decoding the pressure-recovered state through each family's diagnostic decoder makes the recovery visible in physical space (figure~\ref{fig:field_recovery}). T
- **APB-C1** (caption, L94-101) Flow recovered from eight wall-pressure sensors on the representative held-out encounter of figure~\ref{fig:hero}, at the impact-side frame, one row per family.
- **APB-06** (para, L106-113) At the $|G| = 4$ boundary every static approach fails on the load, the via-latent recovery at $\VrecCLGFour$ and the direct no-latent regression at $\DirectCLKe
- **APB-07** (para, L116-127) The window sweeps of \S\ref{sec:res_delays} keep the delay stride at the cache cadence ($\Delta t = \DtTc\,c/u_\infty$), and this choice is a convention rather 
- **APB-08** (para, L133-141) Under a deployment-realism protocol, streaming multi-encounter assimilation with a pressure-only initialisation and no oracle anywhere, the filter on the \PredS
- **APB-C2** (caption, L146-149) Deployment realism (\TestSplit{} set): (a)~streaming multi-encounter filter against sensor noise, three member-noise seeds, validation-selected band (no test-se
- **APB-C3** (caption, L158-162) Relative peak error across the gust envelope (\TestSplit{} set, medians per $|G|$): the forecast-derived filter stays near $12$ to $14$ per cent at all but the 
- **APB-C4** (caption, L170-173) Per-family end-to-end comparison (\TestSplit{} set, $42$ encounters, single filter seed): (a)~phase-resolved lift error per family; (b)~sensor-budget sweep on e
- **APB-09** (para, L182-193) The ladder of \S\ref{sec:res_da} supports explicit selection guidance \citep{gupta2026}. When only the in-distribution median matters and latency is irrelevant,

## sections/appendix_c_calibration.tex  (APC, 3 blocks)
...
- **APC-02** (para, L33-55) The reanalysis reference of \S\ref{sec:methods_rexenkf} is a fixed-lag Rauch--Tung--Striebel smoother running on the linear stack of \S\ref{sec:methods_lae}. A 
- **APC-C1** (caption, L62-70) Estimator calibration and selection (\TestSplit, \ValSplit{} and, through the open markers, \BoundarySplit{} sets; parenthetical counts on the ladder are large-
