"""Session 18 supplementary Figure S: Markov-closure comparison with KRR-RBF probe.

Mirrors Figure 4 (centerpiece, linear ridge) but uses the KernelRidge(RBF)
probe family to read physical observables out of the rolled-out latent.
Shown as supplementary to give reviewers transparency about probe-family
sensitivity. The main paper figure uses linear ridge (Figure 4).

Reads outputs/session18/exp_b1_test3/physical_closure_noBN_krr.csv.
Output: outputs/session18/figures/figureS_markov_closure_krr.{png,pdf}
"""
...
def load_csv(path: Path) -> list[dict]:
    ...
    with open(path) as f:
        for r in csv.DictReader(f):
            r["d"] = int(r["d"]); r["horizon"] = int(r["horizon"]); r["n_enc"] = int(r["n_enc"])
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--csv", type=Path,
        default=Path("outputs/session18/exp_b1_test3/physical_closure_noBN_krr.csv"),
    )
    p.add_argument(
        "--output-png", type=Path,
        default=Path("outputs/session18/figures/figureS_markov_closure_krr.png"),
    )
    p.add_argument(
        "--output-pdf", type=Path,
        default=Path("outputs/session18/figures/figureS_markov_closure_krr.pdf"),
    )
    return p.parse_args()
...
def main() -> None:
    ...
    fig, axes = plt.subplots(
        2, len(PRIMARY_METRICS), figsize=(11, 6.8), sharex=True, constrained_layout=True,
    )
    ...
    for row_idx, split in enumerate(("test_b", "test_c")):
        for col_idx, metric in enumerate(PRIMARY_METRICS):
            ...
            dns_height = float(np.median(dns_heights)) if dns_heights else np.nan

            colors = [BASELINE_COLORS[b] for b in BASELINE_ORDER]
            bars = ax.bar(x, heights, width=bar_w, color=colors, edgecolor="black", linewidth=0.5)
            ...
            if not np.isnan(dns_height):
                ax.axhline(dns_height, color="gray", linestyle=":", linewidth=1.0, alpha=0.7,
            ...
            label_map = {
                "C_L": r"$|\Delta C_L|$",
                "I_y": r"$|\Delta I_y|$",
                "wake_enstrophy": r"$|\Delta E_w|$",
            }
            if col_idx == 0:
                ax.set_ylabel(f"{label_map[metric]} (KRR-RBF probe)", fontsize=10)
            else:
                ax.set_ylabel(label_map[metric], fontsize=10)
            if row_idx == 0:
                title_map = {"C_L": r"$C_L$", "I_y": r"$I_y$",
                             "wake_enstrophy": r"Wake enstrophy $E_w$"}
                ax.set_title(title_map[metric], fontsize=11)
    ...
    fig.suptitle(
        f"Supplementary: Markov closure at H = {H} with nonlinear KernelRidge(RBF) probe\n"
        r"$\hat{O}(t+H)$ = KRR(RBF) probe(rolled-out $z(t+H)$); compare to Figure 4 (linear ridge)",
        fontsize=11, y=1.02,
    )
