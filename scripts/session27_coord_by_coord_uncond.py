...
H = 16
# UNCONDITIONED adaptation: PF z_full is the unconditioned tf-no-c latent; every
# physical descriptor column is DNS-derived (encoder-independent) and copied
# verbatim from the production per_frame_targets file.
...
def main():
    ...
    outp = _P("outputs/session27/causal_noc_tf/coord_by_coord.npz")
    outp.parent.mkdir(parents=True, exist_ok=True)
    np.savez(outp, skill_matrix=M, observables=np.array(OBS), partial_beyond_forces=pj)
    print(f"\n[done] wrote {outp}")
