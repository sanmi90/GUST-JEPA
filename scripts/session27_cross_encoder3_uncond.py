...
import os
H = 16
# UNCONDITIONED adaptation: the JEPA family reads the unconditioned latents
# (default tf-no-c; SURD_MODEL=lstm selects lstm-no-c). Fukami and POD stay at the
# production B1 export so the cross-encoder contrast is against the same baselines
# the manuscript uses. PF descriptors are DNS-derived and encoder-independent, so
# the uncond per_frame_targets copy carries identical physical columns.
...
UNCOND_JEPA = f"outputs/session27/causal_noc_{_MODEL}/latents_jepa_noc/{{}}.npz"
...
def fam_path(fam, sub, split):
    """Resolve the latent path for a family/split: JEPA -> unconditioned export."""
    if fam == "JEPA":
...
def main():
    ...
    for fam, sub in FAMDIR.items():
        r = analyse(load_family(fam_path(fam, sub, "train")),
                    load_family(fam_path(fam, sub, "test_b")),
                    tmap_tr, tmap_te)
        out[fam] = r
    ...
    outp = Path(f"outputs/session27/causal_noc_{_MODEL}/cross_encoder3.json")
