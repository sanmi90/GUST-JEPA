...
# SESSION27 copy lives directly under scripts/, so 'src' is not on sys.path when run
# as `python scripts/...`; add the repo root before importing the pipeline.
...
# SESSION27 UNCONDITIONED VARIANT of scripts/session20/exp_ot_field_and_alignment.py.
# Identical OT/SSIM/alignment method. The predictive (JEPA) decoded fields and latents
# are the fully-unconditioned (no-c) model; the reconstructive (Fukami) reference is
# kept identical to production. NOTE: there is no unconditioned d=32 model, so the
# 'jepa_d32' slot is intentionally pointed at the SAME unconditioned d=64 artifacts
# (its decoded counterpart jepa_d32_norm was likewise filled with the d=64 decode in
# the session27 decode step). The headline jepa_d64-vs-fukami comparison is the
# faithful one; the duplicated jepa_d32 point is labelled as such in the figure.
DECODED_DIR = REPO / "outputs" / "session27" / "decoded_noc_lstm"
OUT_DIR = REPO / "outputs" / "session27" / "ot_noc_lstm"
MANIFEST = REPO / "outputs" / "data_pipeline" / "v1" / "manifest.json"

# Latent sources (z_full = per-frame latent trajectory, (n, 120, d)).
_NOC = REPO / "outputs" / "session27" / "latents_own_jepa_noc_lstm" / "test_b.npz"
LATENTS = {
    "jepa_d64": (_NOC, "z_full"),
    "jepa_d32": (_NOC, "z_full"),  # no unconditioned d=32; reuse d=64 so ordering matches
    "fukami": (
...
def run_d_ii(max_enc: int | None, workers: int) -> dict:
    ...
    # The unconditioned latents use plural keys; production used singular.
    ref = np.load(LATENTS["jepa_d64"][0], allow_pickle=True)
    _ref_cid_key = "case_id" if "case_id" in ref.files else "case_ids"
    _ref_eix_key = "encounter_index" if "encounter_index" in ref.files else "encounter_indices"
    case_ids = [str(c) for c in ref[_ref_cid_key]]
    enc_idx = [int(e) for e in ref[_ref_eix_key]]
