...
# SESSION27 UNCONDITIONED VARIANT of scripts/session20/exp_persistent_homology.py.
# The ONLY behavioural change is the rollout source routing: the predictive (JEPA)
# family points at the fully-unconditioned (no-c) end-to-end rollouts under
# outputs/session27/rollouts_noc_lstm, while the reconstructive (Fukami) reference is
# kept identical to production (rollouts_fukami_d64_noBN under session18/exp_b1_test3).
# Everything downstream (loop_summary, n_significant, lifetime ratio, gate) is
# byte-for-byte the paper's method.
...
OUT_DIR = REPO / "outputs" / "session27" / "persistent_homology_noc_lstm"

# The unconditioned predictive rollouts live at outputs/session27/rollouts_noc_lstm,
# i.e. tag "noc_lstm" under the session27 root. Fukami keeps the production tag/root.
JEPA_TAG = "noc_lstm"
FUKAMI_TAG = "fukami_d64_noBN"

# Per-tag root override: maps a rollout tag to the directory that holds
# rollouts_<tag>/{split}.npz. Any tag not listed falls back to ROLLOUTS_ROOT.
TAG_ROOTS = {
    "noc_tf": SESSION27_ROOT,
    "noc_lstm": SESSION27_ROOT,
}
...
def load_split(tag: str, split: str) -> dict:
    ...
    path = root / f"rollouts_{tag}" / f"{split}.npz"
