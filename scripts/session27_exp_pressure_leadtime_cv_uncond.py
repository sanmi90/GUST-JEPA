"""UNCONDITIONED CV-selected lead-time study (Session 27 copy of
scripts/session21/exp_pressure_leadtime_cv.py).

Both estimators (KRR and LSTM) are selected by 5-fold cross-validation on train
(test never touched), then evaluated across leads, on the fully-unconditioned
(no-c) JEPA d=64 latent. Prints CV-mean R^2 next to test R^2 as the overfitting
check.

Repointing vs production: gather/r2/LEADS come from the session27 unconditioned
lead-time module; outputs go to outputs/session27/pressure_noc_tf/. RTX 6000.
"""
...
# import the UNCONDITIONED lead-time primitives (module lives directly under scripts/)
from session27_exp_pressure_leadtime_uncond import gather, r2, LEADS, LAT  # noqa: E402
from src.utils.device import require_rtx6000  # noqa: E402

OUT = REPO / "outputs/session27/pressure_noc_tf/leadtime.json"
SEL = REPO / "outputs/session27/pressure_noc_tf/leadtime_cv_configs.json"
...
def fit_krr(g, a, Xtr, ytr, Xte):
    ...
    m = KernelRidge(alpha=a, kernel="rbf", gamma=g).fit(
        sx.transform(Xtr), sy.transform(ytr.reshape(len(ytr), -1)))
