...
def main():
    ...
    for fi, fam in enumerate(fams):
        ztr = load_family(B1.format(FAMDIR[fam], "train"))
        zte = load_family(B1.format(FAMDIR[fam], "test_b"))
    ...
    np.savez("outputs_causal/jepa_modes/obs_matrix.npz", matrix=Mat,
             families=np.array(fams), observables=np.array(OBS))
    ...
    out = Path("outputs_causal/jepa_modes/fig_obs_matrix.pdf")
