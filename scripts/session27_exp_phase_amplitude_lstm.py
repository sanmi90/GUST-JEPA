...
# SESSION27 UNCONDITIONED VARIANT of scripts/session20/exp_phase_amplitude.py.
# Identical physics/method; the predictive (JEPA) family is the fully-unconditioned
# (no-c) model: baseline-orbit + test_b DNS-encoded latents come from
# outputs/session27/latents_own_jepa_noc_lstm, the JEPA rollouts from
# outputs/session27/rollouts_noc_lstm. The reconstructive (Fukami) reference is kept
# identical to production. The unconditioned latent npz uses plural keys
# (case_ids/encounter_indices), so key access below is made plural/singular tolerant.
PROD_ROLLOUTS_ROOT = REPO / "outputs" / "session18" / "exp_b1_test3"
SESSION27_ROOT = REPO / "outputs" / "session27"
DNS_LATENTS = SESSION27_ROOT / "latents_own_jepa_noc_lstm"
ROLLOUTS_ROOT = PROD_ROLLOUTS_ROOT  # default root (Fukami lives here)
OUT_DIR = SESSION27_ROOT / "phase_amplitude_noc_lstm"

JEPA_TAG = "noc_lstm"
FUKAMI_TAG = "fukami_d64_noBN"
DT_TC = 0.05  # convective time per frame

# Per-tag rollout root override (unconditioned predictive rollouts live in session27).
TAG_ROOTS = {"noc_tf": SESSION27_ROOT, "noc_lstm": SESSION27_ROOT}


def _key(blob, *names):
    """Return the first present array among names (plural/singular tolerant)."""
    for n in names:
        if n in blob.files:
            return blob[n]
    raise KeyError(f"none of {names} in npz (have {list(blob.files)})")
...
def load_baseline_episodes() -> tuple[np.ndarray, np.ndarray]:
    ...
    cid = _key(d, "case_ids", "case_id").astype(str)
    ei = _key(d, "encounter_indices", "encounter_index")
    mask = cid == "Baseline"
    order = np.argsort(ei[mask])
...
def load_rollout(tag: str) -> dict:
    root = TAG_ROOTS.get(tag, ROLLOUTS_ROOT)
    d = np.load(root / f"rollouts_{tag}" / "test_b.npz", allow_pickle=True)
    return {
...
def main() -> None:
    ...
    cid_tb = _key(dns_tb, "case_ids", "case_id").astype(str)
    ei_tb = _key(dns_tb, "encounter_indices", "encounter_index")
