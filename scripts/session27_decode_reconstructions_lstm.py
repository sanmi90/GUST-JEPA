...
# SESSION27 UNCONDITIONED VARIANT: repointed to the fully-unconditioned (no-c)
# end-to-end JEPA encoder + its dedicated LapFiLM decoder. Only the checkpoint
# paths and the default output dir differ from scripts/session20/decode_reconstructions.py.
ENC_CKPT = REPO / "outputs/session27/JEPA_d64_noc_lstm/checkpoint_iter020000.pt"
DEC_CKPT = REPO / "outputs/session27/decoder_noc_lstm/decoder_iter030000.pt"
# d32 not available for the unconditioned model; reuse the d64 unconditioned pair so
# the optional jepa_d32_norm field is still populated without pointing at production.
ENC_CKPT_D32 = REPO / "outputs/session27/JEPA_d64_noc_lstm/checkpoint_iter020000.pt"
DEC_CKPT_D32 = REPO / "outputs/session27/decoder_noc_lstm/decoder_iter030000.pt"
...
def main():
    ...
    ap.add_argument("--out", type=Path, default=REPO / "outputs/session27/decoded_noc_lstm")
    args = ap.parse_args()
