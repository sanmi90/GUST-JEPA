...
def main() -> None:
    ...
    # Figure: 1 row x 5 columns, mean attribution per tau, plus a 6th panel
    # showing the concentration curve.
    mean_attr_per_tau = []
    for j, t in enumerate(TAUS):
        attrs = np.stack(
    ...
    X_EXT = (-1.5, 4.5)
    Y_EXT = (-1.5, 1.5)
    vmax = max([m.max() for m in mean_attr_per_tau])
    for col, (t, mattr) in enumerate(zip(TAUS, mean_attr_per_tau)):
        ...
        # disk overlay (circle at LE physical (0,0), radius 0.5)
        from matplotlib.patches import Circle
        ax.add_patch(Circle((0, 0), 0.5, fill=False, color="white", lw=1.2))
        # airfoil chord
        ax.plot([0, 1], [0, 0], "k-", lw=2, alpha=0.7)
        ax.set_title(f"tau = {t:+d}\nconc={summary['aggregate_concentration_LE']['mean_per_tau'][col]:.3f}")
        ax.set_xlim(*X_EXT)
        ax.set_ylim(*Y_EXT)
        ax.set_aspect("equal")
    # concentration curve
    ax = axes[5]
    ax.plot(TAUS, mean_per_tau, "o-", label="mean")
    ax.fill_between(
        TAUS,
        np.array(summary["aggregate_concentration_LE"]["min_per_tau"]),
        np.array(summary["aggregate_concentration_LE"]["max_per_tau"]),
        alpha=0.25, label="min/max",
    )
    ax.axvline(0, color="k", lw=1, alpha=0.6)
    ax.set_xlabel(r"tau (frames from impact)")
    ax.set_ylabel(r"frac |attr| within LE disk (r=0.5 c)")
    ax.set_title("LE concentration vs tau")
    ax.legend()
    ax.grid(alpha=0.3)
    fig.suptitle("SHAP-on-Y attribution decay vs frame offset from impact")
    fig.tight_layout()
    fig.savefig(FIGS / "exp3_shap_decay_panels.png", dpi=140, bbox_inches="tight")
    plt.close(fig)
