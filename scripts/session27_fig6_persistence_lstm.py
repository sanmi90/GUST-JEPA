...
# SESSION27 UNCONDITIONED VARIANT of scripts/session21/fig6_persistence.py.
# Imports the machinery from the session27 unconditioned persistence module (same
# ripser, same noise floor, same loop_summary) and routes the predictive (JEPA) tag
# to the unconditioned rollouts while keeping Fukami at the production reference.
REPO = Path(__file__).resolve().parents[1]  # session27 copy lives directly under scripts/
sys.path.insert(0, str(REPO / "scripts" / "session21"))
sys.path.insert(0, str(REPO / "scripts"))
import figstyle as fs  # noqa: E402
from session27_exp_persistent_homology_lstm import (  # noqa: E402
    FUKAMI_TAG, JEPA_TAG, NOISE_FRAC, TAG_ROOTS, ROLLOUTS_ROOT, loop_summary, rips_h1,
)

OUT_PDF = REPO / "outputs/session27/figs_uncond_lstm/fig_persistence.pdf"
OUT_PNG = REPO / "outputs/session27/figs_uncond_lstm/fig_persistence.png"
PVAL = 2.87e-5   # Mann-Whitney one-sided, from session27 persistent_homology_noc_lstm.json


def counts(tag: str):
    root = TAG_ROOTS.get(tag, ROLLOUTS_ROOT)
    z = np.load(root / f"rollouts_{tag}" / "test_b.npz",
