...
# SESSION27 UNCONDITIONED VARIANT of scripts/session20/exp_scale_decomposition.py.
# Identical Gaussian scale-decomposition method; only the decoded-field source and the
# output dir are repointed to the fully-unconditioned (no-c) artifacts. The predictive
# (JEPA) decode = the unconditioned model; fukami/pod columns are the same reconstructive
# references decoded against the unconditioned target (produced by the session27 decode).
REPO = Path(__file__).resolve().parents[1]  # session27 copy lives directly under scripts/
DECODED_ROOT = REPO / "outputs" / "session27" / "decoded_noc_lstm"
MASK_PATH = REPO / "outputs" / "data_pipeline" / "v1" / "airfoil_adjacent_mask.npy"
OUT_DIR = REPO / "outputs" / "session27" / "scale_decomp_noc_lstm"
